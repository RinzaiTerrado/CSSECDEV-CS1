...
public class MgmtUser extends javax.swing.JPanel {
    ...
    public MgmtUser(SQLite sqlite, User userToken) {
        initComponents();
        this.sqlite = sqlite;
...
//      LOAD CONTENTS
    ...
    private void editRoleBtnActionPerformed(java.awt.event.ActionEvent evt) {                                            
        if(table.getSelectedRow() >= 0){
            ...
            if(result != null){
                ...
                sqlite.editUserRole(tableModel.getValueAt(table.getSelectedRow(), 0).toString(), result.charAt(0));
                init();
            }
        }
    }                                           
    ...
    private void chgpassBtnActionPerformed(java.awt.event.ActionEvent evt) {                                           
        if(table.getSelectedRow() >= 0){
            ...
            if (result == JOptionPane.OK_OPTION) {
                ...
                String usernametbale = tableModel.getValueAt(table.getSelectedRow(), 0).toString();
                User selectedUser = sqlite.getUser(usernametbale);
                try{
                    if (PasswordHash.validatePassword(password.getText(), selectedUser.getPassword()) && isValidPassword(confPassString)){
                        String passHash = PasswordHash.createHash(confPassString);
                        //execute sql
                        System.out.println(tableModel.getValueAt(table.getSelectedRow(), 0).toString());
                        sqlite.editUserPassword(tableModel.getValueAt(table.getSelectedRow(), 0).toString(), passHash);
                    } else {
                        System.out.println("not matching or invalid password");
                    }
                }catch(Exception ex) {
